    ...
    public function pre_created_templates() {
        ...
        foreach ($this->database_data() as $value) {
            ...
            $return .= _('<td>
                       ...
                       <button  class="btn btn-success shortcode-addons-addons-data-clone"  style="float:left; margin-right:5px;" type="button" value="' . $id . '">Clone</button>
                       ...
                       <a href="' . esc_url_raw(rest_url()) . 'ShortCodeAddonsUltimate/v2/shortcode_export?styleid=' . $id . '&_wpnonce=' . wp_create_nonce('wp_rest') . '"  title="Export"  class="btn btn-info" style="float:left; margin-right: 5px; margin-left: 5px;">Export</a>
                </td>');
            $return .= _(' </tr>');
        }
        $return .= _('      </tbody>
